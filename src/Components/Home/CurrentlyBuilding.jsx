    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <a
          ...
          className="font-mono text-[11px] tracking-wider text-primary/80 hover:text-primary transition-colors uppercase inline-flex items-center gap-1"
        >
          {event.repo}
          {event.sha && (
            <span className="text-muted-foreground normal-case tracking-normal lowercase">
              · {event.sha.slice(0, 7)}
            </span>
          )}
          <ExternalLink size={9} aria-hidden="true" className="opacity-0 group-hover:opacity-100 transition-opacity" />
        </a>
        <p className="text-sm text-muted-foreground leading-snug mt-0.5 truncate">
          {event.message}
        </p>
      </div>
        ...
        <div className="mb-6 flex items-center gap-3">
          <SectionHeader id="currently-building-heading" title="Currently Building">
            {!loading && !showFallback && (
              <span className="inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded border border-primary/40 text-[11px] font-mono tracking-[0.15em] uppercase text-primary">
            ...
            {[0, 1, 2].map((i) => (
              <div key={i} className="p-4 card animate-pulse">
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 rounded bg-border/60 shrink-0" />
                  <div className="flex-1 space-y-2">
                    <div className="h-3 bg-border/60 rounded w-1/4" />
                    <div className="h-3 bg-border/40 rounded w-3/4" />
                  </div>
                  <div className="h-3 bg-border/40 rounded w-12 shrink-0" />
                </div>
              </div>
            ))}
          </div>
